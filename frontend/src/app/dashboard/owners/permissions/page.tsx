...
export default function OwnersPermissionPage() {
    ...
    (async () => {
      ...
      try {
        const [catalogRes, grantedRes] = await Promise.all([
          ...
          fetch(`${API}/roles/permissions?role=owner`, { headers: headers() }),
        ])
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch(`${API}/roles/permissions?role=owner`, {
  ...
  return (
    <main className="af-db-main">
      <div className="af-db-topbar" style={{ marginBottom: 20 }}>
        <div>
          <h1 className="af-db-greeting" style={{ fontSize: 26 }}>Give Permission To Owner</h1>
        </div>
