...
export default function RenewForm({ leaseId }: { leaseId: number }) {
  ...
  const [form, setForm] = useState({ amount: '', maintenance: '', tax: '', wtax_applicable: false, wtax: '', start_date: '', end_date: '' })
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body?.message || 'Failed to renew lease')
      }
      router.push('/dashboard/leases')
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed to renew lease') }
  ...
  return (
    <main className="af-db-main">
      <div className="af-db-topbar" style={{ marginBottom: 20 }}>
        <div>
          <h1 className="af-db-greeting" style={{ fontSize: 26 }}>Lease Renew</h1>
